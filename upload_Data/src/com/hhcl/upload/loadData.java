...
/*DROP TABLE IF EXISTS `testing`.`new_table_field`;
CREATE TABLE  `testing`.`new_table_field` (
  ...
  `Status` int(10) unsigned default '1001',
  PRIMARY KEY  (`SNO`)
) ENGINE=InnoDB AUTO_INCREMENT=8920 DEFAULT CHARSET=latin1;

*/
data_primary_fetch.append(" SELECT empid,ceil(f1+1) as f1,f2,f3,f4 FROM testing.new_table_field where status in (1001) and f1>0 ");
...
try{
	...
	/*  UpdateITS.append("update hclhrm_prod.tbl_employee_personalinfo set PAN=? ");
	  ...
	  UpdateITS.append("  values(201903,?,?,?,'C',1001)  ");
	...
	/*  UpdateITS.append(" insert into hclhrm_prod.tbl_employee_lop(LOPTRANSACTIONID, BUSINESSUNITID, EMPLOYEEID, LOPCOUNT, STATUS, EMPLOYEESTATUS) ");
	  UpdateITS.append(" values(201806,?,?,?,'C',1001) ");*/
	 /* 
